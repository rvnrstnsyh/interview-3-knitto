...
import TransactionValidationSchema from '../../../../helpers/functions/payload-validations/transaction.ts'
	...
	switch (request.method) {
		...
		case 'POST': {
			...
			try {
				...
				const validation: ReturnType<typeof Http.data> = Http.validator(TransactionValidationSchema, payload)

				if (validation.success && validation.data) {
					ctx.state.transactionData = validation.data
