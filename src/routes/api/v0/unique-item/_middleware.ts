...
import UniqueItemValidationSchema from '../../../../helpers/functions/payload-validations/unique-item.ts'
	...
	switch (request.method) {
		...
		case 'POST': {
			...
			try {
				...
				const validation: ReturnType<typeof Http.data> = Http.validator(UniqueItemValidationSchema, payload)

				if (validation.success && validation.data?.unique_code) {
					ctx.state.unique_code = validation.data.unique_code
